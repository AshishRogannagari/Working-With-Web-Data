...
class SitemapParser:
    ...
    def get_robots_txt(self):
        ...
        """
        try:
            self.sitemaps.extend(line.split(': ')[1].strip() for line in requests.get(f"{self.base_url}/robots.txt").text.split('\n') if line.startswith('Sitemap:'))
    ...
    def parse_sitemaps(self):
        ...
        for sitemap_url in self.sitemaps:
            try:
                ...
                result.append({'Sitemap': sitemap_url, 'URLs': [loc.text for loc in soup.find_all('loc')]})
            except requests.exceptions.RequestException as e:
